...
public class InteractiveSelectQueryExecutor extends QueryExecutor
{
    ...
    public void execute(final Task taskNode, final TaskExecutorContext ctx) {
        ...
        final InteractiveSelectQueryTaskNode task = (InteractiveSelectQueryTaskNode)taskNode;
        final String query = task.getQuery();
        try {
            SharedConnection.execute(reqCtx, reqCtx.getPoolingCriteria(), connection -> {
                InteractiveSelectQueryExecutor.LOGGER.debug("Trying to execute query: {}", (Object)query);
            ...
            InteractiveSelectQueryExecutor.LOGGER.error("Error while trying to execute task {}", (Object)task.getId());
            InteractiveSelectQueryExecutor.LOGGER.error("Exception: {}", (Object)AwbUtil.getErrorMessage((Throwable)e));
            IWRunTimeException.propagate(e, "INTERACTIVE_SELECT_QUERY_EXECUTION_ERROR");
        }
    }
    
    static {
        LOGGER = LoggerFactory.getLogger((Class)InteractiveSelectQueryExecutor.class);
    }
}
