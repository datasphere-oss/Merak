...
public class AnalyticsModelExportExecutor implements TaskExecutor
{
    ...
    public void execute(final Task task, final TaskExecutorContext ctx) {
        ...
        final SparkAnalyticsModelExportTaskNode exportTaskNode = (SparkAnalyticsModelExportTaskNode)task;
        ...
        try {
            ...
            final SparkTranslatorState translatorState = (SparkTranslatorState)exportTaskNode.getTranslatorState();
            translatorState.runTranslations();
            if (reqCtx.getProcessingContext().isValidationMode()) {
                return;
            }
            if (reqCtx.getProcessingContext().getBuildMode() == AwbConstants.BuildMode.INTERACTIVE) {
                final ExecutionResult result = SparkHelper.generateResult(translatorState, (PipelineNode)exportNode, reqCtx.getLimit());
