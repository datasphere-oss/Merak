...
import merak
from merak.serialization.edge import EdgeSchema
from merak.serialization.run_config import RunConfigSchema
from merak.serialization.schedule import ScheduleSchema
from merak.serialization.storage import StorageSchema
from merak.serialization.task import ParameterSchema, TaskSchema
...
def get_parameters(obj: merak.Flow, context: dict) -> List:
...
def get_reference_tasks(obj: merak.Flow, context: dict) -> List:
    if isinstance(obj, merak.Flow):
...
def get_tasks(obj: merak.Flow, context: dict) -> List:
    if isinstance(obj, merak.Flow):
        tasks = obj.tasks
    else:
...
def get_edges(obj: merak.Flow, context: dict) -> List:
    if isinstance(obj, merak.Flow):
        edges = obj.edges
    else:
...
class FlowSchema(ObjectSchema):
    ...
    class Meta:
        object_class = lambda: merak.core.Flow
    ...
    @post_load
    def create_object(self, data: dict, **kwargs: Any) -> merak.core.Flow:
        """
