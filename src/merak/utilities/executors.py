...
from merak.configuration import to_environment_variables
from merak.exceptions import TaskTimeoutSignal, merakSignal
from merak.utilities.logging import get_logger
from typing import (
    TYPE_CHECKING,
    Any,
    Callable,
...
def run_with_heartbeat(
    runner_method: Callable[..., "merak.engine.state.State"]
) -> Callable[..., "merak.engine.state.State"]:
    """
    ...
    @wraps(runner_method)
    def inner(
        self: "merak.engine.cloud.CloudFlowRunner", *args: Any, **kwargs: Any
        ...
        if not use_heartbeat or (merak.context.config.cloud.heartbeat_mode == "off"):
            configured_heartbeat = no_heartbeat()
        elif merak.context.config.cloud.heartbeat_mode == "thread":
            configured_heartbeat = threaded_heartbeat(self.flow_run_id)
        elif merak.context.config.cloud.heartbeat_mode == "process":
...
def subprocess_heartbeat(heartbeat_cmd: List[str], logger: Logger) -> Iterator[None]:
    p = None
    try:
        # we use Popen + a merak CLI for a few reasons:
...
def send_heartbeat(
    flow_run_id: str, client: "merak.client.Client", logger: "logging.Logger"
) -> None:
...
def run_with_thread_timeout(
    ...
    Args:
        ...
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `merak.executors.run_with_thread_timeout` namespace will be created.
...
def multiprocessing_safe_run_and_retrieve(
    ...
    Args:
        ...
        - payload (bytes): A serialized dictionary containing the data required to run
            ...
            - context (dict): The merak context dictionary to use during execution
    ...
    except merakSignal as exc:
        # We need to ensure merakSignals are captured and put in the
        # queue correctly, otherwise no value is returned and
        # the task runner assumes execution has timed out
        return_val = exc
        logger.error(
            f"{name}: Encountered a merakSignal {type(exc).__name__}, "
...
def run_with_multiprocess_timeout(
    ...
    Args:
        ...
        - timeout (int): the length of time to allow for execution before raising a
            `TaskTimeoutSignal`, represented as an integer in seconds
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `merak.` namespace will be created.
        - name (str): an optional name to attach to logs for this function run, defaults
            to the name of the given function. Provides an interface for passing task
            names for logs.
    ...
    try:
        ...
        if isinstance(result, (Exception, merakSignal)):
            raise result
        return result
    except Empty:
...
def run_task_with_timeout(
    ...
    Args:
        ...
        - kwargs (Mapping): keyword arguments to pass to the function
        - logger (Logger): an optional logger to use. If not passed, a logger for the
            `merak.run_task_with_timeout_handler` namespace will be created.
    ...
    """
    logger = logger or get_logger()
    name = merak.context.get("task_full_name", task.name)
    kwargs = kwargs or {}
    ...
    msg = (
        ...
        "consequently merak can only enforce a soft timeout limit, i.e., "
    ...
    fut = executor.submit(run_with_ctx, merak.context.to_dict())
...
def prepare_upstream_states_for_mapping(
    ...
    executor: "merak.executors.Executor",
) -> list:
    """
        ...
        try:
            ...
            for edge, upstream_state in upstream_states.items():
                ...
                # array.
                    ...
                    if (
                        not state.is_mapped()
                        or upstream_state._result != merak.engine.result.NoResult
                    ):
