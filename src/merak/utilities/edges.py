...
class mapped(EdgeAnnotation):
    ...
    Args:
        - value (Any): the task or value to mark as "mapped"; if not a Task
            subclass, merak will attempt to convert it to one when the edge is
            created.

    Example:
        ```python
        from merak import Flow, Task, mapped
...
class unmapped(EdgeAnnotation):
    ...
    Example:
        ```python
        from merak import Flow, Task, unmapped
...
class flatten(EdgeAnnotation):
    ...
    Example:
        ```python
        from merak import Flow, Task, flatten
