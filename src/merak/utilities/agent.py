...
def get_flow_image(flow_run: GraphQLResult, default: str = None) -> str:
    ...
    Args:
        ...
        - default (str, optional): A default image to use. If not specified,
            The `merakhq/merak` image corresponding with the flow's merak
    ...
    # - Image on run_config
    # - Provided default
    # - `merakhq/merak` for flow's core version
    ...
    return f"merakhq/merak:{cleaned_version}"
...
def get_flow_run_command(flow_run: GraphQLResult) -> str:
    ...
    Returns:
        - str: a merak CLI command to execute a flow run
    """
    core_version = getattr(flow_run.flow, "core_version", None) or "0.0.0"

    if parse(core_version) < parse("0.13.0"):
        return "merak execute cloud-flow"

    return "merak execute flow-run"
