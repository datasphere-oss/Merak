...
def system_information() -> dict:
    ...
    return dict(
        system_information=dict(
            ...
            merak_version=merak.__version__,
            merak_backend=merak.config.backend,
        )
    )
...
def config_overrides(include_secret_names: bool = False) -> dict:
    ...
    *not* returned.
    ...
    default_config = dict()
    default_config_path = merak.configuration.DEFAULT_CONFIG
    if default_config_path and os.path.isfile(default_config_path):
        default_config = merak.configuration.load_toml(default_config_path)

    user_config = dict()  # type: ignore
    user_config_path = merak.configuration.USER_CONFIG
    if user_config_path and os.path.isfile(
        str(merak.configuration.interpolate_env_vars(user_config_path))
    ):
        user_config = merak.configuration.load_toml(user_config_path)

    # Create some shorter names for fully specified imports avoiding circular
    # dependencies in the utilities
    dict_to_flatdict = merak.utilities.collections.dict_to_flatdict
    flatdict_to_dict = merak.utilities.collections.flatdict_to_dict
...
def environment_variables() -> dict:
    """
    Get `merak__` specific environment variables

    Returns:
        - dict: a dictionary containing names of set merak environment variables
    """
    env_vars = list()
    for env_var, _ in os.environ.items():
        if env_var.startswith(merak.configuration.ENV_VAR_PREFIX + "__"):
            env_vars.append(env_var)

    return dict(env_vars=env_vars)


def flow_information(flow: "merak.Flow") -> dict:
    """
    Get flow information

    Args:
        - flow ("merak.Flow"): the flow whose attributes to retrieve
...
def diagnostic_info(
    flow: "merak.Flow" = None, include_secret_names: bool = False
) -> str:
    """
    Get full diagnostic information

    Args:
        - flow ("merak.Flow", optional): the flow whose attributes to retrieve. If no
