...
import merak
from merak.exceptions import FlowStorageError

if TYPE_CHECKING:
    from merak.core.flow import Flow  # pylint: disable=W0611
...
def get_flow_image(flow: "Flow") -> str:
    ...
    This function was deprecated in 0.15.0 and `merak.utilities.agent.get_flow_image`
    ...
    if run_config is not None and hasattr(run_config, "image"):
        ...
        if not isinstance(storage, merak.storage.Docker):
            raise ValueError(
...
def extract_flow_from_file(
    ...
    flows = {o.name: o for o in exec_vals.values() if isinstance(o, merak.Flow)}
    if flows:
        if flow_name:
...
def extract_flow_from_module(module_str: str, flow_name: str = None) -> "Flow":
    ...
    if obj_name_present:
        ...
        if callable(flow):
            flow = flow()
        if not isinstance(flow, merak.Flow):
            raise TypeError(
                f"Object at {module_str!r} is a {type(flow)} not a `merak.Flow`"
            )
...
def flow_from_bytes_pickle(data: bytes) -> "Flow":
    ...
    try:
        ...
        # Serialized using older version of merak, use cloudpickle directly
        flow_bytes = data
        reg_versions = {}
    else:
    ...
    run_merak = run_versions["merak"]
    reg_merak = reg_versions.get("merak")
    if reg_merak and parse(reg_merak) != parse(run_merak):
        # If we didn't error above, still check that the merak versions match
        # and warn if they don't. merak version mismatches *may* work, but
        # they may also error later leading to confusing behavior.
        warnings.warn(
            f"This flow was built using merak {reg_merak!r}, but you currently "
            f"have merak {run_merak!r} installed. We recommend loading flows "
            "with the same merak version they were built with, failure to do so "
