...
class ObjectSchema(Schema):
    ...
    def _add_version(self, data: dict, **kwargs: Any) -> dict:
        ...
        # don't mutate data
        data = data.copy()
        data.setdefault("__version__", merak.__version__)
        return data
    ...
    def _serialize(self, value, attr, obj, **kwargs):  # type: ignore
        try:
            ...
            raise ValidationError(
                "When running with merak Cloud/Server, values for "
...
class StatefulFunctionReference(fields.Field):
    ...
    def _serialize(self, value, attr, obj, **kwargs):  # type: ignore
        ...
        if not valid_bases and self.reject_invalid:
            raise ValidationError(
                "When running with merak Cloud/Server, custom functions aren't supported "
