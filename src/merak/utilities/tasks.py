...
if TYPE_CHECKING:
    import merak.tasks.core.constants
    import merak.tasks.core.collections
    import merak.tasks.core.function
...
def apply_map(func: Callable, *args: Any, flow: "Flow" = None, **kwargs: Any) -> Any:
    ...
    `merak.unmapped`. Nested arguments that should be unnested before mapping
    over should be wrapped with `merak.flatten`
    ...
    ```python
    from merak import task, case, apply_map
    from merak.tasks.control_flow import merge
    ...
    ```
    """
    from merak.tasks.core.constants import Constant
    ...
    assert isinstance(flow, merak.Flow)  # appease mypy
    ...
    for x in itertools.chain(args, kwargs.values()):
        if not isinstance(
            x, (merak.Task, merak.utilities.edges.EdgeAnnotation, Sequence)
        ):
            raise TypeError(
                f"Cannot map over non-sequence object of type `{type(x).__name__}`"
            )

    flow2 = merak.Flow("temporary flow")
    ...
    def preprocess(a: Any) -> "merak.Task":
        # Clear external case/resource when adding tasks to flow2
        with merak.context(case=None, resource=None):
            a2 = as_task(a, flow=flow2)
            is_mapped = not isinstance(a, merak.utilities.edges.unmapped)
            is_constant = isinstance(a2, Constant)
            is_flattened = isinstance(a, merak.utilities.edges.flatten)
            if not is_constant:
    ...
    with merak.context(mapped=True, case=None, resource=None):
        with flow2:
            if no_flow_provided:
...
def tags(*tags: str) -> Iterator[None]:
    ...
    """
    tags_set = set(tags)
    tags_set.update(merak.context.get("tags", set()))
    with merak.context(tags=tags_set):
        yield


def as_task(x: Any, flow: "Optional[Flow]" = None) -> "merak.Task":
    """
    ...
    Args:
        - x (object): any Python object to convert to a merak Task
        - flow (Flow, optional): Flow to which the merak Task will be bound

    Returns:
        - a merak Task representing the passed object
    """
    from merak.tasks.core.constants import Constant
    ...
    def is_constant(x: Any) -> bool:
        ...
        """
        if isinstance(x, (merak.core.Task, merak.utilities.edges.EdgeAnnotation)):
            return False
    ...
    # task objects
    if isinstance(x, merak.core.Task):  # type: ignore
        return x
    elif isinstance(x, merak.utilities.edges.EdgeAnnotation):
        return as_task(x.value, flow=flow)

    # handle constants, including collections of constants
    elif is_constant(x):
        return_task = Constant(x)  # type: merak.core.Task

    # collections
    elif isinstance(x, list):
        return_task = merak.tasks.core.collections.List().bind(*x, flow=flow)
    elif isinstance(x, tuple):
        return_task = merak.tasks.core.collections.Tuple().bind(*x, flow=flow)
    elif isinstance(x, set):
        return_task = merak.tasks.core.collections.Set().bind(*x, flow=flow)
    elif isinstance(x, dict):
        keys, values = [], []
        for k, v in x.items():
            keys.append(k)
            values.append(v)
        return_task = merak.tasks.core.collections.Dict().bind(
...
def pause_task(message: str = None, duration: timedelta = None) -> None:
    ...
    Example:
        ```python
        from merak import Flow
        from merak.utilities.tasks import task, pause_task
    ...
    if duration is not None:
        ...
        start_time = None
    if merak.context.get("resume", False) is False:
        raise merak.engine.signals.PAUSE(  # type: ignore
...
@overload
def task(__fn: Callable) -> "merak.tasks.core.function.FunctionTask":
    pass
...
) -> Callable[[Callable], "merak.tasks.core.function.FunctionTask"]:
    pass
...
def task(
    ...
    "merak.tasks.core.function.FunctionTask",
    Callable[[Callable], "merak.tasks.core.function.FunctionTask"],
]:
    """
    ...
    The decorator is best suited to merak's functional API, but can also be used
    ...
    """
    if fn is None:
        return lambda fn: merak.tasks.core.function.FunctionTask(
            fn=fn,
            **task_init_kwargs,
        )
    return merak.tasks.core.function.FunctionTask(fn=fn, **task_init_kwargs)
