...
Note that Merak Tasks come equipped with their own loggers.  These can be accessed via:
    - `self.logger` if implementing a Task class
    - `merak.context.get("logger")` if using the `task` decorator

When running locally, log levels and message formatting are set via your merak configuration file.
"""
import atexit
import logging
import sys
...
class LogManager:
    """A global log manager for managing all logs to be sent to merak"""

    def __init__(self) -> None:
        self.queue = Queue()  # type: Queue[dict]
        self.pending_logs = []  # type: List[dict]
        self.thread = None  # type: Optional[threading.Thread]
        self.client = None  # type: Optional[merak.Client]
...
class CloudHandler(logging.Handler):
    """A handler for sending logs to the merak API"""
...
def _log_record_context_injector(*args: Any, **kwargs: Any) -> logging.LogRecord:
    ...
    for attr in merak_LOG_RECORD_ATTRIBUTES + tuple(additional_attrs):
...
def configure_logging(testing: bool = False) -> logging.Logger:
    """
    Creates a "merak" root logger with a `StreamHandler` that has level and formatting
    set from `merak.config`.
    ...
    Args:
        - testing (bool, optional): a boolean specifying whether this configuration
            ...
            by configuring a "merak-test-logger" instead of the standard "merak" logger

    Returns:
        - logging.Logger: a configured logging object
    """
    name = "merak-test-logger" if testing else "merak"

    return _create_logger(name)


context.logger = merak_logger = configure_logging()


def configure_extra_loggers() -> None:
    """
    Creates a "merak" configured logger for all strings in extra_loggers config list.
...
def create_diagnostic_logger(name: str) -> logging.Logger:
    ...
    merak logging configuration.  For diagnostic / debugging / internal use only.
    """
...
def get_logger(name: str = None) -> logging.Logger:
    """
    Returns a "merak" logger.

    Args:
        - name (str): if `None`, the root merak logger is returned. If provided, a child
            logger of the name `"merak.{name}"` is returned. The child logger inherits
    ...
    if name is None:
        ...
        return merak_logger.getChild(name)
...
def temporary_logger_config(
    ...
    """
    Set a temporary config for the `merak` logger. The formatting can be updated
    ...
    Yields:
        The configured logger; also affects any `merak` loggers in use
    """
    logger = get_logger()
    ...
    try:
        with merak.utilities.configuration.set_temporary_config(overrides):
...
class RedirectToLog:
    ...
    def write(self, s: str) -> None:
        """
        Write message from stdout to a merak logger.
