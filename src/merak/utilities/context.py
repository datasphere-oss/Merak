"""
This module implements the merak context that is available when tasks run.

Tasks can import `merak` and access `merak.context` values that will be set
...
with merak.context(dict(a=1, b=2)):
    print(merak.context.a) # 1

print(merak.context.get("a")) # undefined
```

merak provides various key / value pairs in context that are always available during task runs:
...
| `config` | the complete [merak configuration]\
(https://docs.merak.io/core/concepts/configuration.html) object that is being used \
during this run |
...
merak Cloud and Server supply some additional context variables:
...
the [Context concept
doc](https://docs.merak.io/core/concepts/execution.html#context).
"""
...
from merak.utilities.collections import DotDict, merge_dicts


class Context(DotDict, threading.local):
    """
    A thread safe context store for merak data.
    ...
    def __getstate__(self) -> None:
        ...
        as an attribute of the merak module.
        """
        raise TypeError(
            "Pickling context objects is explicitly not supported. You should always "
            "access context as an attribute of the `merak` module, as in `merak.context`"
        )
    ...
    def __call__(self, *args: MutableMapping, **kwargs: Any) -> Iterator["Context"]:
        """
        A context manager for setting / resetting the merak context

        Example:
            import merak.context
            with merak.context(dict(a=1, b=2), c=3):
                print(merak.context.a) # 1
        """
