...
def jira_message_formatter(
    tracked_obj: "Union[Flow, Task]", state: "merak.engine.state.State"
) -> str:
    time = datetime.utcnow()
    msg = "Message from merak: {0} entered a {1} state at {2}".format(
...
def jira_notifier(
    ...
    token ('JIRATOKEN') should be set as part of a 'JIRASECRETS' object in merak Secrets.
    ...
    Example:
        ...
        from merak.utilities.jira_notification import jira_notifier
    ...
    try:
        ...
        raise ImportError(
            'Using `jira_notifier` requires merak to be installed with the "jira" extra.'
        ) from exc

    options = options or dict()
    jira_credentials = cast(dict, merak.client.Secret("JIRASECRETS").get())
