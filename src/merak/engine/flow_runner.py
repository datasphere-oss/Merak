...
import pendulum
import merak
from merak.core import Edge, Flow, Task
from merak.engine.result import Result
from merak.engine.results import ConstantResult
...
from merak.utilities import executors
from merak.utilities.collections import flatten_seq
...
class FlowRunner(Runner):
    ...
    def __init__(
        ...
        if task_runner_cls is None:
            task_runner_cls = merak.engine.get_default_task_runner_class()
    ...
    def run(
        ...
        run_context = dict(merak.context)
        ...
        if executor is None:
            ...
            if executor is None:
                executor = merak.engine.get_default_executor_class()()
        ...
        try:
            ...
            with merak.context(run_context):
    ...
    def get_flow_run_state(
        ...
        executor: "merak.executors.base.Executor",
    ) -> State:
        """
        Runs the flow.
        ...
        Args:
            ...
            - executor (Executor): executor to use when performing computation; defaults to the
                executor provided in your merak configuration
        ...
        with self.check_for_cancellation(), executor.start():
            ...
            for task in self.flow.sorted_tasks():
                ...
                if task_state is None and isinstance(
                    task, merak.tasks.core.constants.Constant
                ):
                ...
                if (
                    isinstance(
                        task,
                        (
                            merak.tasks.core.resource_manager.ResourceSetupTask,
                            merak.tasks.core.resource_manager.ResourceCleanupTask,
                            merak.tasks.secrets.SecretBase,
                        ),
                    )
                ...
                for key, val in self.flow.constants[task].items():
                    edge = Edge(
                        upstream_task=merak.tasks.core.constants.Constant(val),
                ...
                if any(edge.mapped for edge in upstream_states.keys()):
                    ...
                    task_states[task] = executor.wait(
                        executor.submit(
                            ...
                            context=dict(
                                merak.context, **task_contexts.get(task, {})
                            ),
                    ...
                    task_states[task] = executor.submit(
                        ...
                        state=task_state,
                        upstream_states=upstream_states,
                        context=dict(merak.context, **task_contexts.get(task, {})),
                        flow_result=self.flow.result,
                        task_runner_cls=self.task_runner_cls,
                        task_runner_state_handlers=task_runner_state_handlers,
                        upstream_mapped_states=upstream_mapped_states,
