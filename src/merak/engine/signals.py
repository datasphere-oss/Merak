...
from merak.engine import state
from merak.exceptions import MerakSignal


def signal_from_state(state: state.State) -> Type["MerakStateSignal"]:
    """
    Given a state instance, returns the corresponding Signal type.

    Args:
        - state (State): an instance of a merak State

    Returns:
        - merakStateSignal: the merak Signal corresponding to
            the provided state

    Raises:
        - ValueError: if no signal matches the provided state
    """
    unprocessed = set(MerakStateSignal.__subclasses__())
    signals = dict()
    while unprocessed:
        sig = unprocessed.pop()
...
class merakStateSignal(MerakSignal):
    """
    Create a new merakStateSignal object.
...
class LOOP(MerakStateSignal):
    """
    Indicates that a task should loop with the provided result.  Note that the result
    included in the `LOOP` signal will be available in merak context during the next iteration
    ...
    from merak.engine.signals import LOOP

    @task
    def accumulate(x: int) -> int:
        current = merak.context.get("task_loop_result", x)
        if current < 100:
...
class SUCCESS(MerakStateSignal):
    """
