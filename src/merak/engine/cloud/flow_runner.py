...
from merak.core import Flow, Task
from merak.engine.cloud import CloudTaskRunner
from merak.engine.flow_runner import FlowRunner, FlowRunnerInitializeResult
from merak.engine.runner import ENDRUN
from merak.engine.state import Failed, Queued, State, Cancelling, Cancelled
from merak.exceptions import VersionLockMismatchSignal
...
class CloudFlowRunner(FlowRunner):
    ...
    def _heartbeat(self) -> bool:
        try:
            ...
            self.flow_run_id = merak.context.get("flow_run_id", "")  # type: str
    ...
    def call_runner_target_handlers(self, old_state: State, new_state: State) -> State:
        ...
        Returns:
            - State: the new state
        """
        raise_on_exception = merak.context.get("raise_on_exception", False)
        ...
        flow_run_id = merak.context.get("flow_run_id", None)
        version = merak.context.get("flow_run_version")
        ...
        merak.context.update(flow_run_version=(version or 0) + 1)
    ...
    def check_for_cancellation(self) -> Iterator:
        ...
        def interrupt_if_cancelling() -> None:
            ...
            with merak.context(context):
                flow_run_id = merak.context["flow_run_id"]
                while True:
                    exiting_context = done.wait(
                        merak.config.cloud.check_cancellation_interval
                    )
                    try:
        ...
        try:
            ...
            thread.join()
            if cancelling:
                merak.context.update(flow_run_version=flow_run_version)
    ...
    def run(
        ...
        executor: "merak.executors.Executor" = None,
        ...
        Args:
            ...
            - executor (Executor, optional): executor to use when performing
                computation; defaults to the executor specified in your merak configuration
            - context (Dict[str, Any], optional): merak.Context to use for execution
        ...
        while end_state.is_queued() and (
            ...
            while time_remaining > 0:
                delay = min(
                    merak.config.cloud.check_cancellation_interval, time_remaining
                )
    ...
    def initialize_run(  # type: ignore
        ...
        try:
            ...
            if state is None:
                state = Failed(
                    message="Could not retrieve state from merak Cloud", result=exc
                )
            raise ENDRUN(state=state) from exc
        ...
        for task_run in flow_run_info.task_runs:
            ...
            try:
                ...
                raise KeyError(
                    ...
                    "stored in the merak backend and the flow that was loaded from "
                    "storage.\n"
