...
from merak.engine.serializers import JSONSerializer, Serializer, DateTimeSerializer


class MerakResult(Result):
    """
    Hook for storing and retrieving JSON serializable Python objects that can
    safely be stored directly in a merak database.
    ...
    def serializer(self, val: Serializer) -> None:  # type: ignore
        if not isinstance(val, (JSONSerializer, DateTimeSerializer)):
            raise TypeError(
                "merakResult only supports JSONSerializer or DateTimeSerializer"
            )
