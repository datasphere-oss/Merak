    ...
    **Note**: If this result raises a `PermissionError` that could mean it is attempting
    ...
    Args:
        - dir (str, optional): the _absolute_ path to a directory for storing
            all results; defaults to `${merak.config.home_dir}/results`
    ...
    def __init__(
        ...
        full_merak_path = os.path.abspath(config.home_dir)
        common_path = ""
        try:
            if dir is not None:
                common_path = os.path.commonpath(
                    [full_merak_path, os.path.abspath(dir)]
                )
        except ValueError:
            # ValueError is raised if comparing two paths in Windows from different drives,
            # e.g., E:/ and C:/
            pass
        if dir is None or common_path == full_merak_path:
    ...
    def default_location(self) -> str:
        fname = "merak-result-" + slugify(pendulum.now("utc").isoformat())
