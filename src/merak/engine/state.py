"""
State is the main currency in the merak platform. It is used to represent the current
status of a flow or task.

This module contains all merak state classes, all ultimately inheriting from the base State
class as follows:
...
import merak
from merak.engine.result import Result, NoResult
...
class State:
    ...
    **Note:** Each state-checking method (e.g., `is_failed()`) will also return `True`
    ...
    def __init__(
        ...
        if "task_tags" in merak.context:
            self.context.setdefault("tags", list(merak.context.task_tags))
    ...
    def __getstate__(self) -> dict:
        ...
        if isinstance(self.result, Exception) and not isinstance(
            self.result, merak.engine.signals.merakStateSignal
        ):
...
class Scheduled(Pending):
    ...
    def __init__(
        ...
        run_count = merak.context.get("task_run_count")
...
class ClientFailed(_MetaState):
    """
    The `ClientFailed` state is used to indicate that the merak Client failed
...
class Looped(Finished):
    ...
    def __init__(
        ...
        if loop_count is None:
            loop_count = merak.context.get("task_loop_count", 1)
