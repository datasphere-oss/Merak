...
import merak
from merak.engine.results import LocalResult
...
class Local(Storage):
    ...
    Note that if you register a Flow with merak Cloud using this storage,
    ...
    Args:
        - directory (str, optional): the directory the flows will be stored in;
            defaults to `~/.merak/flows`.  If it doesn't already exist, it will be
            created for you.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        directory = directory or os.path.join(merak.config.home_dir, "flows")
