...
import merak
from merak.engine.results import GCSResult
...
class GCS(Storage):
    ...
    that your merak Agent has the proper credentials available (see
    ...
    Args:
        ...
        - local_script_path (str, optional): the path to a local script to upload when `stored_as_script`
            is set to `True`. If not set then the value of `local_script_path` from `merak.context` is
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.local_script_path = local_script_path or merak.context.get(
    ...
    def _gcs_client(self):  # type: ignore
        from merak.utilities.gcp import get_storage_client
