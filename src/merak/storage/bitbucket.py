...
import merak
from merak.client import Secret
from merak.storage import Storage
from merak.utilities.storage import extract_flow_from_file
...
class Bitbucket(Storage):
    ...
    - Call `merak register -f flow.py` to register this flow with Bitbucket storage.
    ...
    Args:
        ...
        - access_token_secret (str, optional): the name of a merak secret
            that contains a Bitbucket access token to use when loading flows from
            this storage. Bitbucket Server only
        - cloud_username_secret (str, optional): the name of a merak secret that contains a
            Bitbucket username to use when loading flows from this storage. Bitbucket Cloud only.
        - cloud_app_password_secret (str, optional): the name of a merak secret that contains a
    ...
    def _get_bitbucket_server_client(self) -> "atlassian.Bitbucket":
        ...
        if self.access_token_secret is not None:
            ...
            access_token = merak.context.get("secrets", {}).get(
