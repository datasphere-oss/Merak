...
from merak.utilities.storage import extract_flow_from_module
...
class Module(Storage):
    """
    A merak Storage class for referencing flows that can be imported from a
    python module.
    ...
    merak flows. If this module is installed and available in your execution
    environment you can use `Module` storage to reference and load the flows.

    ```python
    from merak import Flow
    from merak.storage import Module
    ...
    def add_flow(self, flow: "Flow") -> str:
        ...
        Args:
            - flow (Flow): the merak Flow to add.
