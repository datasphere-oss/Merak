...
import merak
from merak.storage import Storage
from merak.utilities.storage import (
...
class Docker(Storage):
    """
    Docker storage provides a mechanism for storing merak flows in Docker
    ...
    Args:
        ...
        - base_image (str, optional): the base image for this when building this
            image (e.g. `python:3.7`), defaults to the `merakhq/merak` image
            matching your python version and merak core library version used
            at runtime.
        ...
        - merak_version (str, optional): an optional branch, tag, or commit
            specifying the version of merak you want installed into the container;
        ...
        - merak_directory (str, optional): Path to the directory where merak configuration/flows
             should be stored inside the Docker image. Defaults to `/opt/merak`.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.merak_directory = merak_directory
        self.path = path

        self.env_vars = env_vars or {}
        self.env_vars.setdefault(
            "merak__USER_CONFIG_PATH", "{}/config.toml".format(self.merak_directory)
        )
        ...
        if merak_version is None:
            ...
            self.merak_version = merak_version

        if base_image is None and dockerfile is None:
            python_version = "{}.{}".format(
                sys.version_info.major, sys.version_info.minor
            )
            if re.match(r"^[0-9]+\.[0-9]+\.[0-9]+$", self.merak_version) is not None:
                self.base_image = "merakhq/merak:{}-python{}".format(
                    self.merak_version, python_version
                )
            elif self.merak_version.endswith("rc0"):
                # Development release candidate
                self.base_image = f"merakhq/merak:{self.merak_version}"
            elif (
                re.match(r"^[0-9]+\.[0-9]+rc[0-9]+$", self.merak_version) is not None
            ):
        ...
        # we should always try to install merak, unless it is already installed. We can't
        # determine this until image build time.
        self.installation_commands.append(
            f"pip show merak || "
            f"pip install git+https://github.com/merakHQ/merak.git"
            f"@{self.merak_version}#egg=merak[all_orchestration_extras]"
        )
    ...
    def add_flow(self, flow: "merak.core.flow.Flow") -> str:
        """
        Method for adding a new flow to this Storage object.

        Args:
            - flow (Flow): a merak Flow to add
        ...
        flow_path = self.path or "{}/flows/{}.merak".format(
            self.merak_directory, slugify(flow.name)
        )
        self.flows[flow.name] = flow_path
        self._flows[flow.name] = flow  # needed prior to build
        return flow_path

    def get_flow(self, flow_name: str) -> "merak.core.flow.Flow":
        """
    ...
    def create_dockerfile_object(self, directory: str) -> str:
        ...
        *Note*: if `files` are added to this container, they will be copied to this
        ...
        if not self.ignore_healthchecks:
            ...
            healthcheck_run = (
                f"RUN python {self.merak_directory}/healthcheck.py "
        ...
        file_contents = textwrap.dedent(
            ...
            RUN mkdir -p {self.merak_directory}/
            {multiline_indent(copy_flows, 12)}
            COPY {healthcheck_loc} {self.merak_directory}/healthcheck.py
