...
from merak import Client, config
from merak.exceptions import AuthorizationError, ClientError
from merak.cli.build_register import handle_terminal_error, TerminalError
from merak.backend import TenantView
...
def check_override_auth_token():
    ...
    if config.cloud.get("auth_token"):
        if os.environ.get("merak__CLOUD__AUTH_TOKEN"):
            click.secho(
                "Auth token has been set in the environment. The CLI cannot be used to "
                "manage your auth. Unset the key with `unset merak__CLOUD__AUTH_TOKEN`",
                fg="red",
            )
        else:
            click.secho(
...
def abort_on_config_api_key(message: str = None):
    if config.cloud.get("api_key"):
        ...
        raise TerminalError(
            "Your API key is set in the merak config instead of with the CLI."
            + message
        )
...
def auth():
    """
    Handle merak Cloud authorization.
    ...
    \b
    Commands:
        login           Log in to merak Cloud
        logout          Log out of merak Cloud
    ...
    \b    $ merak auth switch-tenants --slug test-person

    \b  Create a new API key that expires at the start of the next year

    \b    $ merak auth create-key -n marvin --expire 2022-1-1
    """
    if config.backend == "server":
        raise click.UsageError(
            "merak Server does not have authentication. Change your backend to "
            "merak Cloud with `merak backend cloud` to log in."
        )
...
@click.option(
    "--key",
    "-k",
    help="A merak Cloud API key.",
)
@handle_terminal_error
def login(key):
    """
    Login to merak Cloud

    Create an API key in the UI then login with it here:

        $ merak auth login -k YOUR-KEY

    You will be switched to the default tenant associated with the key. After login,
    your available tenants can be seen with `merak auth list-tenants` and you can
    change the default tenant on this machine using `merak auth switch-tenants`.

    The given key will be stored on disk for later access. merak will default to using
    ...
    abort_on_config_api_key(
        "To log in with the CLI, remove the config key `merak.cloud.api_key`"
    )
    ...
    try:
        ...
        raise TerminalError("Unauthorized. Invalid merak Cloud API key.")
    except ClientError:
        raise TerminalError("Error attempting to communicate with merak Cloud.")
    else:
        client.tenant_id = tenant_id
        client.save_auth_to_disk()
        tenant = TenantView.from_tenant_id(tenant_id)
        click.secho(
            f"Logged in to merak Cloud tenant {tenant.name!r} ({tenant.slug})",
            fg="green",
        )
        return
...
def logout(token):
    ...
    if client.api_key and not token:
        ...
        abort_on_config_api_key(
            "To log out, remove the config key `merak.cloud.api_key`"
        )

        click.confirm(
            "Are you sure you want to log out of merak Cloud? "
        ...
        click.secho("Logged out of merak Cloud", fg="green")

    else:
        raise TerminalError(
            "You are not logged in to merak Cloud. "
            "Use `merak auth login` to log in first."
        )
...
def list_tenants():
    ...
    try:
        ...
        raise TerminalError(
            "You are not authenticated. Use `merak auth login` first."
        )
...
@auth.command(hidden=True)
@click.option(
    "--id", "-i", required=False, help="A merak Cloud tenant id.", hidden=True
)
@click.option(
    "--slug", "-s", required=False, help="A merak Cloud tenant slug.", hidden=True
)
@click.option(
    "--default",
    "-d",
...
def create_key(name, expire, quiet):
    """
    Create a merak Cloud API key for authentication with your current user
    """
    ...
    if not user_id:
        raise TerminalError("Failed to retrieve the current user id from merak Cloud")
    ...
    if key is None:
        raise TerminalError(f"Unexpected response from merak Cloud: {response}")
...
def status():
    """
    Get the current merak authentication status
    """
    client = Client()
    ...
    if config.cloud.get("auth_token"):
        if os.environ.get("merak__CLOUD__AUTH_TOKEN"):
            click.secho(
                ...
                "ignored. We recommend unsetting the 'merak__CLOUD__AUTH_TOKEN' key",
                fg="yellow",
            )
        else:
            click.secho(
                "An authentication token is set via the merak config file. "
                "These have been removed in favor of API keys and the setting will be "
                "ignored. We recommend removing the 'merak.cloud.auth_token' key",
                fg="yellow",
            )

    if not client.api_key:
        click.secho(
            "You are not logged in! Use `merak auth login` to login with an API key.",
            fg="yellow",
        )
