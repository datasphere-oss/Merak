...
COMMON_START_OPTIONS = [
    click.option(
        ...
        "-k",
        help=(
            "A merak Cloud API key. If not set, the value will be inferred from the "
            "local machine."
        ),
    ),
    click.option(
    ...
    click.option(
        ...
        "-a",
        required=False,
        help="A merak API URL. If not set, the value in the config is used.",
    ),
    click.option(
        "--agent-config-id",
    ...
    click.option(
        "--max-polls",
        help=(
            "Maximum number of times the agent should poll the merak API for flow "
    ...
    click.option(
        ...
        help=(
            ...
            "merak config value will be used."
        ),
        default=None,
    ),
    click.option(
...
def start(import_paths, **kwargs):
    """Start a local agent"""
    from merak.agent.local import LocalAgent
...
def start(volumes, **kwargs):
    ...
    from merak.agent.docker import DockerAgent
...
@agent.group()
def kubernetes():
    """Manage merak Kubernetes agents."""
...
def start(image_pull_secrets=None, **kwargs):
    ...
    from merak.agent.kubernetes import KubernetesAgent
...
@kubernetes.command()
@add_options(COMMON_INSTALL_OPTIONS)
@click.option("--api", "-a", required=False, help="A merak API URL.")
...
@click.option("--latest", is_flag=True, help="Use the latest merak image.")
@click.option("--mem-request", help="Requested memory for merak init job.")
@click.option("--mem-limit", help="Limit memory for merak init job.")
@click.option("--cpu-request", help="Requested CPU for merak init job.")
@click.option("--cpu-limit", help="Limit CPU for merak init job.")
@click.option("--image-pull-policy", help="imagePullPolicy for merak init job")
@click.option(
    "--service-account-name", help="Name of Service Account for merak init job"
)
@click.option("--backend", "-b", help="merak backend to use for this agent.")
...
def start(**kwargs):
    """Start an ECS agent"""
    from merak.agent.ecs import ECSAgent
...
def start(**kwargs):
    ...
    from merak.agent.vertex.agent import VertexAgent
