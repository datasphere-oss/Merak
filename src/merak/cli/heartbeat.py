...
def heartbeat():
    """
    Send heartbeats back to the merak API.

    \b
    Usage:
        $ merak heartbeat [task-run/flow-run] -i ID
...
def flow_run(id, num):
    """
    Send heartbeats back to the merak API for a given flow run ID.
    ...
    # console stdout/err
    with merak.context({"flow_run_id": id, "running_with_backend": True}):
