...
from merak.utilities.cli import add_options


@click.group(hidden=True)
def server():
    """
    Commands for interacting with the merak Core server

    \b
    Usage:
        $ merak server [COMMAND]

    \b
    Arguments:
        start                   Start merak Server using docker-compose
        stop                    Stop merak Server by removing all containers and networks
    ...
    Examples:
        ...
        $ merak server config > docker-compose.yml
    """
...
def setup_compose_env(
    ...
    base_version = merak.__version__.split("+")
    ...
    merak_ENV = dict(
        ...
        merak_CORE_VERSION=merak.__version__,
        # Set the server image tag
        merak_SERVER_TAG=version or default_tag,
    )

    APOLLO_ENV = dict(
        HASURA_API_URL=f"http://hasura:{hasura_port}/v1alpha1/graphql",
        HASURA_WS_URL=f"ws://hasura:{hasura_port}/v1alpha1/graphql",
        merak_API_URL=f"http://graphql:{graphql_port}{config.server.graphql.path}",
        merak_API_HEALTH_URL=f"http://graphql:{graphql_port}/health",
        APOLLO_HOST_PORT=str(server_port),
        APOLLO_HOST_IP="0.0.0.0" if expose else config.server.host_ip,
        merak_SERVER__TELEMETRY__ENABLED=(
    ...
    UI_ENV = dict(
        APOLLO_URL=config.server.ui.apollo_url, merak_UI_TAG=ui_version or default_tag
    )
    ...
    if "merak_UI_TAG" in env and ui_version and env["merak_UI_TAG"] != ui_version:
        warnings.warn(
            "The UI version has been set in the environment via `merak_UI_TAG` and "
            f"the CLI. The environment variable value (={env['merak_UI_TAG']}) will "
            "be ignored."
        )

    if "merak_SERVER_TAG" in env and version and env["merak_SERVER_TAG"] != version:
        warnings.warn(
            "The version has been set in the environment via `merak_SERVER_TAG` and "
            f"the CLI. The environment variable value (={env['merak_SERVER_TAG']}) "
            "will be ignored."
        )

    if "merak_SERVER_DB_CMD" in env and no_upgrade:
        warnings.warn(
            "The database startup command has been set in the environment via "
            "`merak_SERVER_DB_CMD` and `--no-upgrade` has been passed via CLI. The "
            f"environment variable value (={env['merak_SERVER_DB_CMD']}) will be "
            "ignored."
        )
        env.pop("merak_SERVER_DB_CMD")

    # Allow a more complex database command to be passed via env
    env.setdefault(
        "merak_SERVER_DB_CMD",
        "merak-server database upgrade -y"
        if not no_upgrade
        else "echo 'DATABASE MIGRATIONS SKIPPED'",
    )

    env.update(**merak_ENV, **APOLLO_ENV, **POSTGRES_ENV, **UI_ENV, **HASURA_ENV)
...
def config_cmd(
    ...
    \b
        ...
        --volume-path       TEXT    A path to use for the Postgres volume, defaults to
                                    '~/.merak/pg_data'. Not valid for external Postgres.
    """
...
def start(
    ...
    """
    This command spins up all infrastructure and services for the merak Core server
    ...
    try:
        ...
        started = False
        with merak.utilities.configuration.set_temporary_config(
            {
...
def ascii_welcome(ui_port="8080"):
    ...
    docs_url = click.style("https://docs.merak.io", fg="white", bg="blue", bold=True)
...
def stop():
    """
    This command stops all merak Server containers that are connected to the
    `merak-server` network. Note: This will completely remove the `merak-server` network
    ...
    networks = client.networks(names=["merak-server"])

    if not networks:
        click.echo("No running merak Server found")
        return
    network_id = networks[0].get("Id")

    click.echo("Stopping merak Server containers and network")
    ...
    click.echo("merak Server stopped")
...
def create_tenant(name, slug):
    """
    This command creates a tenant for the merak Server
