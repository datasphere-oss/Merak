...
from merak.backend.flow_run import FlowRunView, watch_flow_run
from merak.backend.execution import execute_flow_run_in_subprocess
from merak.cli.build_register import (
    TerminalError,
...
from merak.utilities.logging import temporary_logger_config
...
def load_flows_from_script(path: str) -> "List[merak.Flow]":
    ...
    # TODO: This is copied and slightly modified from `merak.cli.build_register`
    ...
    try:
        with merak.context({"loading_flow": True, "local_script_path": path}):
...
See `merak run --help` for more details on the options.
"""
...
@click.option(
    "--project",
    help="The name of the merak project containing the flow to run.",
)
@click.option(
    "--path",
    "-p",
...
def run(
    ...
    if path or module:
        ...
        with temporary_logger_config(
            ...
            with merak.context(**context_dict):
                try:
    ...
    if log_level:
        ...
        if not run_config.env:
            run_config.env = {}
        run_config.env["merak__LOGGING__LEVEL"] = log_level
    else:
        run_config = None
    ...
    try:  # Handle keyboard interrupts during creation
        ...
        with try_error_done(
            ...
            flow_run_id = client.create_flow_run(
                ...
                # https://github.com/merakHQ/server/blob/77c301ce0c8deda4f8771f7e9991b25e7911224a/src/merak_server/api/runs.py#L136
