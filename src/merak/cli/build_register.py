...
import merak
from merak.utilities.filesystems import read_bytes_from_path, parse_path
from merak.utilities.graphql import with_args, EnumValue, compress
from merak.utilities.importtools import import_object
from merak.storage import Local, Module
from merak.run_configs import UniversalRun


FlowLike = Union[box.Box, "merak.Flow"]
...
def load_flows_from_script(path: str) -> "List[merak.Flow]":
    ...
    try:
        with merak.context({"loading_flow": True, "local_script_path": abs_path}):
    ...
    flows = [f for f in namespace.values() if isinstance(f, merak.Flow)]
    if flows:
        for f in flows:
...
def load_flows_from_module(name: str) -> "List[merak.Flow]":
    """
    Given a module name (or full import path to a flow), load all flows found in the
    module
    """
    try:
        with merak.context({"loading_flow": True}):
    ...
    if isinstance(mod_or_obj, ModuleType):
        flows = [f for f in vars(mod_or_obj).values() if isinstance(f, merak.Flow)]
    elif isinstance(mod_or_obj, merak.Flow):
        flows = [mod_or_obj]
...
def load_flows_from_json(path: str) -> "List[dict]":
    ...
    try:
        ...
        raise TerminalError(f"{path!r} is not a valid merak flows `json` file.")
...
def get_project_id(client: "merak.Client", project: str) -> str:
...
def register_serialized_flow(
    ...
    Args:
        - client (merak.Client): the merak client
...
def build_and_register(
    ...
    Args:
        - client (merak.Client): the merak client to use
    ...
    for flow in flows:
        storage = flow.storage if isinstance(flow, merak.Flow) else None
...
@click.option(
    "--project",
    help="The name of the merak project to register this flow in. Required.",
    default=None,
)
@click.option(
    "--path",
...
@click.option(
    ...
    "json_paths",
    help=(
        "A path or URL to a JSON file created by `merak build` containing the flow(s) "
...
def build(paths, modules, names, labels, output, update):
    ...
    the `--json` flag to `merak register`.
    """
    succeeded = 0
    errored = 0
