...
import merak
from merak.utilities import backend as backend_util
...
def cli():
    """
    The merak CLI for creating, managing, and inspecting your flows.

    \b
    Note: a merak Cloud API token is required for all Cloud related commands. If a token
    is not set then run `merak auth login` to set it.
    ...
    \b
    Setup Commands:
        auth        Handle merak Cloud authorization
        backend     Switch between `server` and `cloud` backends
        server      Interact with the merak Server

    \b
    Miscellaneous Commands:
        version     Print the current merak version
        config      Output merak config
        diagnostics Output merak diagnostic information
    """
    pass
...
def repl():
    """
    Start an interactive merak CLI session
    """
    banner = f"""
Welcome to the merak REPL
version : {merak.__version__}
    """
...
"""
    ...
    history_path = os.path.join(merak.config.home_dir, "repl-history")
    prompt_kwargs = {
...
def version():
    """
    Get the current merak version
    """
    click.echo(merak.__version__)
...
def config():
    ...
    Output merak config
    """
    click.echo(merak.config.to_json())


@cli.command(hidden=True)
@click.option(
    "--include-secret-names",
    help="Output merak diagnostic information",
    hidden=True,
    is_flag=True,
)
def diagnostics(include_secret_names):
    """
    Output merak diagnostic information
    ...
    """
    click.echo(
        merak.utilities.diagnostics.diagnostic_info(
...
def backend(api):
    """
    Switch merak API backend to either `server` or `cloud`
    """
