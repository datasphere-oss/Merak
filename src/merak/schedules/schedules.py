...
class Schedule:
    ...
    from merak.schedules import Schedule, filters
    from merak.schedules.clocks import IntervalClock
    ...
    Args:
        - clocks (List[merak.schedules.clocks.Clock]): one or more clocks that emit events
    ...
    def __init__(
        self,
        clocks: List["merak.schedules.clocks.Clock"],
    ...
    def _get_clock_events(
    ...
    ) -> Iterable["merak.schedules.clocks.ClockEvent"]:
        """
        ...
        )  # type: Iterable[merak.schedules.clocks.ClockEvent]
...
def IntervalSchedule(
    ...
    return Schedule(
        clocks=[
            merak.schedules.clocks.IntervalClock(
...
def CronSchedule(
    ...
    *the first time* 2am is reached, 120 minutes later. Longer clocks, such as one
    ...
    return Schedule(
        clocks=[
            merak.schedules.clocks.CronClock(
...
def RRuleSchedule(
    ...
    """
    return Schedule(
        clocks=[merak.schedules.clocks.RRuleClock(rrule_obj, start_date, end_date)]
    )
