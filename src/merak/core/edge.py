...
class Edge:
    ...
    Args:
        ...
        - flow (merak.Flow, optional): a flow object that will be used
    ...
    def __init__(
        ...
        mapped: bool = None,
        flattened: bool = None,
        flow: "merak.core.flow.Flow" = None,
    ):
        ...
        annotations = {}
        if isinstance(upstream_task, merak.utilities.edges.EdgeAnnotation):
            annotations.update(upstream_task.annotations)
        if isinstance(downstream_task, merak.utilities.edges.EdgeAnnotation):
            annotations.update(downstream_task.annotations)
        upstream_task = merak.utilities.tasks.as_task(upstream_task, flow=flow)
        downstream_task = merak.utilities.tasks.as_task(downstream_task, flow=flow)
    ...
    def serialize(self) -> dict:
        ...
        """
        return merak.serialization.edge.EdgeSchema().dump(self)
