...
from merak.core.edge import Edge
from merak.core.parameter import Parameter
from merak.core.task import Task
from merak.executors import Executor
from merak.engine.result import Result
from merak.engine.state import State
from merak.storage import Storage, get_default_storage_class
from merak.run_configs import RunConfig, UniversalRun
from merak.utilities import diagnostics, logging
from merak.utilities.configuration import set_temporary_config
...
class Flow:
    ...
    Args:
        ...
        - schedule (merak.schedules.Schedule, optional): A default schedule for the flow
        - executor (merak.executors.Executor, optional): The executor that the flow
           should use. If `None`, the default executor configured in the runtime environment
           will be used.
        - run_config (merak.run_configs.RunConfig, optional): The runtime
           configuration to use when deploying this flow.
        - storage (merak.storage.Storage, optional): The unit of storage
    ...
    def __init__(
        self,
        name: str,
        schedule: merak.schedules.Schedule = None,
        ...
        self._merak_version = merak.__version__
    ...
    def replace(self, old: Task, new: Task, validate: bool = True) -> None:
        ...
        Args:
            ...
            - new (Task): the new task to replace the old with; if not a merak
                Task, merak will attempt to convert it to one
    ...
    def _flow_context(self) -> Iterator["Flow"]:
        """
        When entering a flow context, the merak context is modified to include:
        ...
        with merak.context(flow=self, _unused_task_tracker=unused_task_tracker):
            yield self
        ...
        if unused_task_tracker.difference(self.tasks):
            warnings.warn(
                ...
                "https://docs.merak.io/core/advanced_tutorials/"
    ...
    def _default_reference_tasks(self) -> Set[Task]:
        from merak.tasks.core.resource_manager import (
            ResourceInitTask,
            ResourceCleanupTask,
        )
    ...
    def _generate_task_slug(self, task: Task) -> str:
        ...
        the merak API uses to track state updates for each task within a Flow.

        The logic for slug generation within a Flow is essentially "name-tags-#count".  Having
        slugs be properties of (Task, Flow) instead of just Task alone allows merak to ensure
    ...
    def add_task(self, task: Task) -> Task:
        ...
        if not isinstance(task, Task):
            ...
            if not isinstance(task, (Parameter, merak.tasks.core.constants.Constant)):
                case = merak.context.get("case", None)
                if case is not None:
                    case.add_task(task, self)
                resource = merak.context.get("resource", None)
    ...
    def add_edge(
        ...
        Args:
            ...
            - validate (bool, optional): Whether or not to check the validity of the flow
                ...
                `eager_edge_validation` in your merak configuration file.

        Returns:
            - merak.core.edge.Edge: The `Edge` object that was successfully added to the flow
        ...
        """
        if validate is None:
            validate = cast(bool, merak.config.flows.eager_edge_validation)

        if mapped and merak.context.get("mapped", False):
            raise ValueError(
        ...
        if (
            isinstance(edge.upstream_task, merak.tasks.core.constants.Constant)
            and edge.key
            and not edge.mapped
    ...
    def set_dependencies(
        ...
        Args:
            - task (Any): a Task that will become part of the Flow. If the task is not a
                Task subclass, merak will attempt to convert it to one.
            - upstream_tasks ([Any], optional): Tasks that will run before the task runs. If
                any task is not a Task subclass, merak will attempt to convert it to one.
            - downstream_tasks ([Any], optional): Tasks that will run after the task runs.
                If any task is not a Task subclass, merak will attempt to convert it to one.
            - keyword_tasks ({key: Any}, optional): The results of these tasks
                will be provided to the task under the specified keyword
                arguments. If any task is not a Task subclass, merak will attempt to
                convert it to one.
            - mapped (bool, optional): Whether the upstream tasks (both keyed
                ...
                tasks wrapped in the `merak.utilities.edges.unmapped` container will
                _not_ be mapped over.
            - validate (bool, optional): Whether or not to check the validity of the flow
                (e.g., presence of cycles).  Defaults to the value of `eager_edge_validation`
                in your merak configuration file.
    ...
    def _run(
        ...
        **kwargs: Any,
        ...
        flow_state = merak.engine.state.Scheduled(start_time=next_run_time, result={})
        ...
        while True:
            ...
            if not error:
                ...
                for t, s in flow_state.result.items():
                    ...
                    fresh_states = [
                        s
                        for s in merak.context.caches.get(t.cache_key or t.name, [])
                        + cached_sub_states
                        if s.cached_result_expiration
                        and s.cached_result_expiration > now
                    ]
                    merak.context.caches[t.cache_key or t.name] = fresh_states
            ...
            flow_state = merak.engine.state.Scheduled(
    ...
    def run(
        ...
        **kwargs: Any,
    ) -> Union["merak.engine.state.State", None]:
        """
        ...
        if merak.context.get("loading_flow", False):
            warnings.warn(
        ...
        if runner_cls is None:
            runner_cls = merak.engine.get_default_flow_runner_class()
        ...
        merak.context.setdefault("caches", {})

        if run_on_schedule is None:
            run_on_schedule = cast(bool, merak.config.flows.run_on_schedule)
        ...
        for task in self.tasks or []:
            if task not in state.result:
                state.result[task] = merak.engine.state.Pending(
    ...
    def visualize(
        self,
        flow_state: "merak.engine.state.State" = None,
        filename: str = None,
        format: str = None,
        ...
        try:
            ...
            msg = (
                ...
                "Try re-installing merak with `pip install 'merak[viz]'`"
            )
        ...
        for t in self.tasks:
            ...
            # See: https://github.com/merakHQ/merak/issues/5656
    ...
    def serialize(self, build: bool = False) -> dict:
        ...
        self.validate()
        schema = merak.serialization.flow.FlowSchema
    ...
    def diagnostics(self, include_secret_names: bool = False) -> str:
        """
        Get flow and merak diagnostic information
    ...
    def load(cls, fpath: str) -> "Flow":
        ...
        """
        from merak.utilities.storage import flow_from_bytes_pickle

        if not os.path.isabs(fpath):
            path = "{home}/flows".format(home=merak.context.config.home_dir)  # type: ignore
            fpath = Path(os.path.expanduser(path)) / "{}.merak".format(  # type: ignore
                slugify(fpath)
            )  # type: ignore
    ...
    def save(self, fpath: str = None) -> str:
        ...
        Args:
            - fpath (str, optional): the filepath where your Flow will be saved; defaults to
                `~/.merak/flows/FLOW-NAME.merak`

        Returns:
            - str: the full location the Flow was saved to
        """
        from merak.utilities.storage import flow_to_bytes_pickle

        if fpath is None:
            path = "{home}/flows".format(home=merak.context.config.home_dir)  # type: ignore
            fpath = Path(  # type: ignore
                os.path.expanduser(path)  # type: ignore
            ) / "{}.merak".format(  # type: ignore
                slugify(self.name)
            )
    ...
    def run_agent(
        ...
        Args:
            - token (str, optional): A merak Cloud API token with a RUNNER scope;
            ...
            - log_to_cloud (bool, optional): a boolean specifying whether Flow run logs should
                be sent to merak Cloud; defaults to `None` which uses the config value
            - api_key (str, optional): A merak Cloud API key to authenticate with.
                If not set, the default will be pulled from the `Client`
        """
        temp_config = {
            "cloud.agent.auth_token": token or merak.config.cloud.agent.auth_token,
            "cloud.api_key": api_key or merak.config.cloud.get("api_key"),
            "cloud.send_flow_run_logs": (
                log_to_cloud
                if log_to_cloud is not None
                else merak.config.cloud.send_flow_run_logs
            ),
        }
        with set_temporary_config(temp_config):
            if self.run_config is not None:
                ...
                labels = []
            agent = merak.agent.local.LocalAgent(
    ...
    def register(
        ...
        **kwargs: Any,
        ...
        """
        Register the flow with merak Cloud; if no storage is present on the Flow, the default
        ...
        Args:
            ...
            - labels (List[str], optional): a list of labels to add to this Flow;
                ...
                http://docs.merak.io/orchestration/agents/overview.html#labels
