...
from merak.run_configs import ECSRun
...
class ECSAgent(Agent):
    ...
    Args:
        ...
        - name (str, optional): An optional name to give this agent. Can also
            be set through the environment variable `merak__CLOUD__AGENT__NAME`.
            Defaults to "agent".
        - labels (List[str], optional): A list of labels, which are arbitrary
            string identifiers used by merak Agents when polling for work.
        ...
        - max_polls (int, optional): Maximum number of times the agent will
            poll merak Cloud for flow runs; defaults to infinite.
        ...
        - no_cloud_logs (bool, optional): Disable logging to a merak backend
    ...
    def deploy_flow(self, flow_run: GraphQLResult) -> str:
        ...
        if run_config.task_definition_arn is None:
            ...
            from merak.serialization.storage import StorageSchema
            from merak.storage import Docker
    ...
    def generate_task_definition(
        ...
        slug = slugify.slugify(
            ...
            max_length=255 - len("merak-"),
            word_boundary=True,
            save_order=True,
        )
        taskdef["family"] = f"merak-{slug}"

        # Add some metadata tags for easier tracking by users
        taskdef.setdefault("tags", []).extend(
            [
                {"key": "merak:flow-id", "value": flow_run.flow.id},
                {"key": "merak:flow-version", "value": str(flow_run.flow.version)},
            ]
        )
        ...
        # Add `merak__CONTEXT__IMAGE` environment variable
        env = {"merak__CONTEXT__IMAGE": image}
    ...
    def get_run_task_kwargs(
        ...
        # Add `merak__LOGGING__LEVEL` environment variable
