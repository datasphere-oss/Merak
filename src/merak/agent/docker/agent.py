...
from merak import config, context
from merak.agent import Agent
from merak.run_configs import DockerRun
...
class DockerAgent(Agent):
    ...
    Docker Agent can be found at https://docs.merak.io/orchestration/agents/docker.html
    ...
    ```
    merak agent docker start --env MY_SECRET_KEY=secret --env OTHER_VAR=$OTHER_VAR
    ```

    The default Docker daemon may be overridden by providing a different `base_url`:
    ```
    merak agent docker start --base-url "tcp://0.0.0.0:2375"
    ```

    If connecting to a local instance of merak Server that does not have the
    `--expose` flag set, you must provide a network:
    ```
    merak agent docker start --network merak-server
    ```
    ...
    def deploy_flow(self, flow_run: GraphQLResult) -> str:
        ...
        labels = {
            "io.merak.flow-name": flow_run.flow.name,
            "io.merak.flow-id": flow_run.flow.id,
            "io.merak.flow-run-id": flow_run.id,
        }
    ...
    def populate_env_vars(
        ...
        if "localhost" in config.cloud.api:
            if self.networks and "merak-server" in self.networks:
        ...
        env.update(
            {
                ...
                "merak__CONTEXT__FLOW_RUN_ID": flow_run.id,  # type: ignore
                "merak__CONTEXT__FLOW_ID": flow_run.flow.id,  # type: ignore
