...
from merak.agent import Agent
from merak.engine.state import Failed
from merak.run_configs import KubernetesRun
from merak.utilities.agent import get_flow_image, get_flow_run_command
from merak.exceptions import ClientError, ObjectNotFoundError
from merak.utilities.filesystems import read_bytes_from_path
from merak.utilities.graphql import GraphQLResult
...
class KubernetesAgent(Agent):
    ...
    https://docs.merak.io/orchestration/agents/kubernetes.html
    ...
    Args:
        ...
        - name (str, optional): An optional name to give this agent. Can also be set through
            the environment variable `merak__CLOUD__AGENT__NAME`. Defaults to "agent"
        - labels (List[str], optional): a list of labels, which are arbitrary string
            identifiers used by merak Agents when polling for work
        - env_vars (dict, optional): a dictionary of environment variables and values that will
            ...
            own environment
        - max_polls (int, optional): maximum number of times the agent will poll merak Cloud
        ...
        - agent_address (str, optional):  Address to serve internal API at. Currently this is
            ...
            (default).
        - no_cloud_logs (bool, optional): Disable logging to a merak backend for this agent
        ...
        - delete_finished_jobs (bool, optional): A boolean to toggle if finished merak jobs
    ...
    def __init__(
        ...
        from merak.utilities.kubernetes import get_kubernetes_client
        ...
        while more:
            try:
                jobs = self.batch_client.list_namespaced_job(
                    ...
                    label_selector="merak.io/identifier",
                    limit=20,
                    _continue=_continue,
                )
                ...
                for job in jobs.items:
                    ...
                    flow_run_id = job.metadata.labels.get("merak.io/flow_run_id")
                    ...
                    if not delete_job:
                        pods = self.core_client.list_namespaced_pod(
                            namespace=self.namespace,
                            label_selector="merak.io/identifier={}".format(
                                job.metadata.labels.get("merak.io/identifier")
                            ),
                        )
    ...
    def generate_job_spec(self, flow_run: GraphQLResult) -> dict:
        ...
        job_name = f"merak-job-{identifier}"

        # Populate job metadata for identification
        k8s_labels = {
            "merak.io/identifier": identifier,
            "merak.io/flow_run_id": flow_run.id,  # type: ignore
            "merak.io/flow_id": flow_run.flow.id,  # type: ignore
        }
        ...
        # Get the first container, which is used for the merak job
        ...
        env = {"merak__LOGGING__LEVEL": config.logging.level}
        env.update(self.env_vars)
        if run_config.env:
            env.update(run_config.env)
        env.update(
            {
                "merak__BACKEND": config.backend,
                "merak__CLOUD__AGENT__LABELS": str(self.labels),
                "merak__CLOUD__API": config.cloud.api,
                "merak__CLOUD__API_KEY": self.flow_run_api_key or "",
                "merak__CLOUD__TENANT_ID": (
                    # A tenant id is only required when authenticating
                    self.client.tenant_id
                    if self.flow_run_api_key
                    else ""
                ),
                "merak__CLOUD__USE_LOCAL_SECRETS": "false",
                "merak__CLOUD__SEND_FLOW_RUN_LOGS": str(self.log_to_cloud).lower(),
                "merak__CONTEXT__FLOW_RUN_ID": flow_run.id,
                "merak__CONTEXT__FLOW_ID": flow_run.flow.id,
                "merak__CONTEXT__IMAGE": image,
                "merak__ENGINE__FLOW_RUNNER__DEFAULT_CLASS": "merak.engine.cloud.CloudFlowRunner",
                "merak__ENGINE__TASK_RUNNER__DEFAULT_CLASS": "merak.engine.cloud.CloudTaskRunner",
                # Backwards compatibility variable for containers on merak <0.15.0
                "merak__LOGGING__LOG_TO_CLOUD": str(self.log_to_cloud).lower(),
                # Backwards compatibility variable for containers on merak <1.0.0
                "merak__CLOUD__AUTH_TOKEN": self.flow_run_api_key or "",
            }
        )
    ...
    def generate_deployment_yaml(
        ...
        Args:
            - api (str, optional): A URL pointing to the merak API. Defaults to
                `https://api.merak.io`
            - namespace (str, optional): The namespace to create merak jobs in. Defaults
                to `default`
            ...
            - latest (bool, optional): Whether to use the `latest` merak image.
                Defaults to `False`
            - mem_request (str, optional): Requested memory for merak init job.
            - mem_limit (str, optional): Limit memory for merak init job.
            - cpu_request (str, optional): Requested CPU for merak init job.
            - cpu_limit (str, optional): Limit CPU for merak init job.
            - image_pull_policy (str, optional): imagePullPolicy to use for merak init job.
                Job defaults to `IfNotPresent`.
            - service_account_name (str, optional): Name of a service account to use for
                merak init job. Job defaults to using `default` service account.
            - labels (List[str], optional): a list of labels, which are arbitrary string
                identifiers used by merak Agents when polling for work
        ...
        version = merak.__version__.split("+")
        image_version = (
        ...
        if env_vars:
            agent_env.append(
                {
                    "name": "merak__CLOUD__AGENT__ENV_VARS",
                    "value": json.dumps(env_vars),
                }
            )

        # Use local merak version for image
        deployment["spec"]["template"]["spec"]["containers"][0][
            "image"
        ] = "merakhq/merak:{}".format(image_version)
