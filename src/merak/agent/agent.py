...
from merak.engine.state import Failed, Submitted
from merak.run_configs import RunConfig, UniversalRun
from merak.serialization.state import StateSchema
from merak.serialization.run_config import RunConfigSchema
from merak.utilities.context import context
from merak.utilities.graphql import GraphQLResult, with_args
...
class Agent:
    """
    Base class for Agents. Information on using the merak agents can be found at
    https://docs.merak.io/orchestration/agents/overview.html

    This Agent class is a standard point for executing Flows through the merak API. It is meant to
    ...
    Flow on the given platform. It is built in this way to keep merak API logic standard
    ...
    Args:
        ...
        - max_polls (int, optional): maximum number of times the agent will poll the merak API
    ...
    def __init__(
        ...
        self.logger.debug(f"merak backend: {config.backend}")
    ...
    def _get_ready_flow_runs(self, prefetch_seconds: int = 10) -> Set[str]:
        """
        Query the merak API for flow runs in the 'ready' queue. Results from here
    ...
    def _setup_api_connection(self) -> None:
        ...
        self.client.attach_headers({"X-merak-AGENT-ID": self._register_agent()})
    ...
    def _show_startup_display(self) -> None:
        ...
        self.logger.info(
            ...
            "https://docs.merak.io/orchestration/"
        )
