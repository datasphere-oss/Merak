...
from merak.utilities import collections

DEFAULT_CONFIG = os.path.join(os.path.dirname(__file__), "config.toml")
USER_CONFIG = os.getenv("merak__USER_CONFIG_PATH", "~/.merak/config.toml")
BACKEND_CONFIG = os.getenv("merak__BACKEND_CONFIG_PATH", "~/.merak/backend.toml")
ENV_VAR_PREFIX = "merak"
...
def to_environment_variables(
    config: Config, include: Optional[Iterable[str]] = None, prefix: str = "merak"
) -> dict:
    """
    ...
    Args:
        ...
        - prefix: The prefix for the environment variables. Defaults to "merak".

    Returns:
        - A dictionary mapping key to values e.g.
            merak__SECTION__KEY: VALUE
    """
