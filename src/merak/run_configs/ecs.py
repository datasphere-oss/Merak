...
from merak.run_configs.base import RunConfig
from merak.utilities.filesystems import parse_path
...
class ECSRun(RunConfig):
    ...
    Note that certain merak specific environment variables defined within the
    ...
    although the variable `merak__LOGGING__LEVEL` might be defined within
    `containerDefinitions` (of the `task_definition`), it will first be
    ovewritten with the merak config and then by `env` (if defined). Therefore,
    do not set any merak specific environment variables within `task_definition`,
    ...
    Args:
        - task_definition (dict, optional): An in-memory task definition spec
            ...
            a container named `flow` isn't part of the task definition, merak
            ...
            directly in merak Cloud/Server - use `task_definition_path`
        ...
        - task_definition_path (str, optional): Path to a task definition spec
            ...
            `merak agent ecs start --label your-label`), then you can reference a path
        ...
        - labels (Iterable[str], optional): An iterable of labels to apply to this
            run config. Labels are string identifiers used by merak Agents
    ...
    called "merakEcsCluster".

    ```python
    flow.run_config = ECSRun(
        labels=["prod"],
        task_definition_path="s3://bucket/flow_task_definition.yaml",
        run_task_kwargs=dict(cluster="merakEcsCluster"),
    )
    ```
    ...
    cpu: 1024
    memory: 2048
    taskRoleArn: arn:aws:iam::XXX:role/merakTaskRole
    executionRoleArn: arn:aws:iam::XXX:role/merakECSAgentTaskExecutionRole
    ...
    - name: flow
        ...
        logDriver: awslogs
        options:
            awslogs-group: "/ecs/merakEcsAgent"
    ...
    or by a full ARN), then deploy the flow to a cluster called "merakEcsCluster":

    ```python
    flow.run_config = ECSRun(
        labels=["prod"],
        task_definition_arn="merakFlow:1",
        # or using a full ARN:
        # task_definition_arn="arn:aws:ecs:us-east-1:XXX:task-definition/merakFlow:1"
        run_task_kwargs=dict(cluster="merakEcsCluster"),
    )
    ```

    4) Let merak register a new task definition for a flow with S3 storage and a custom
    ECR image:

    ```python
    flow.run_config = ECSRun(
        labels=["prod"],
        task_role_arn="arn:aws:iam::XXXX:role/merakTaskRole",
        execution_role_arn="arn:aws:iam::XXXX:role/merakECSAgentTaskExecutionRole",
    ...
    ```python
    import merak
    from merak.storage import S3
    from merak.run_configs import ECSRun
    from merak import task, Flow
    from merak.client.secrets import Secret
    ...
    RUN_CONFIG = ECSRun(
        ...
        task_definition=dict(
            ...
            cpu=1024,
            memory=2048,
            taskRoleArn=f"arn:aws:iam::{ACCOUNT_ID}:role/merakTaskRole",
            executionRoleArn=f"arn:aws:iam::{ACCOUNT_ID}:role/merakECSAgentTaskExecutionRole",
            containerDefinitions=[
                dict(
                    name="flow",
    ...
    @task
    def say_hi():
        logger = merak.context.get("logger")
        logger.info("Hi from merak %s from flow %s", merak.__version__, FLOW_NAME)
