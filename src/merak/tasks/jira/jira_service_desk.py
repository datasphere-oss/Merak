...
class JiraServiceDeskTask(Task):
    ...
    token ('JIRATOKEN') can be set as part of a 'JIRASECRETS' object in merak Secrets.
    ...
    Args:
        - server_url (str): the URL of your atlassian account e.g.
            "https://test.atlassian.net".  Can also be set as a merak Secret.
    ...
    def run(
        ...
        Args:
            - username(str): the jira username, provided with a merak secret (defaults to
                JIRAUSER in JIRASECRETS)
            - access_token (str): a Jira access token, provided with a merak secret (defaults
                to JIRATOKEN in JIRASECRETS)
            - server_url (str): the URL of your atlassian account e.g.
                "https://test.atlassian.net".  Can also be set as a merak Secret. Defaults to
        ...
        try:
            ...
            raise ImportError(
                'Using Jira tasks requires merak to be installed with the "jira" extra.'
            ) from exc
