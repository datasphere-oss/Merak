"""
Tasks for creating and inspecting Merak flow runs
...
import merak
from merak import Client, Task, task
from merak.backend.artifacts import create_link_artifact
from merak.backend.flow_run import FlowRunView, FlowView, watch_flow_run
from merak.client import Client
from merak.engine.signals import signal_from_state
from merak.run_configs import RunConfig
from merak.utilities.graphql import EnumValue, with_args
...
def create_flow_run(
    ...
    """
    Task to create a flow run in the merak backend.
    ...
    if not run_name:
        current_run = merak.context.get("flow_run_name")
        if current_run:
    ...
    if idempotency_key is None:
        ...
        # to `None`
        idempotency_key = merak.context.get("task_run_id")
        map_index = merak.context.get("map_index")
...
def get_task_run_result(
    ...
    if merak.context.get("flow_run_id") == flow_run_id:
...
def wait_for_flow_run(
    ...
    for log in watch_flow_run(
        ...
        merak.context.logger.log(log.level, message)
...
class StartFlowRun(Task):
    """
    Task used to kick off a flow run using merak Core's server or merak Cloud.  If multiple
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if poll_interval.total_seconds() < 3:
            raise ValueError(
                "`poll_interval` needs to be at least 3 seconds to avoid spamming the merak server. "
    ...
    def run(
        ...
        if idempotency_key is None:
            idempotency_key = merak.context.get("task_run_id", None)
