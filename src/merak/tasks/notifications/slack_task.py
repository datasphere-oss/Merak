...
class SlackTask(Task):
    ...
    merak Secret set which stores your Slack webhook URL.  For installing the merak App,
    please see these [installation
    instructions](https://docs.merak.io/core/advanced_tutorials/slack-notifications.html#installation-instructions).

    Args:
        - message (str, optional): the message to send as either a dictionary or a plain
            string; can also be provided at runtime
        - webhook_secret (str, optional): the name of the merak Secret which stores your
    ...
    def run(
        ...
        Args:
            ...
            - webhook_url (str, optional): the value of a Slack webhook URL that is returned from an
                upstream `merakSecret` task. If specified then the `webhook_secret` will not be used.
