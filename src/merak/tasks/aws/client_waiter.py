...
from merak.tasks.aws import waiters
...
class AWSClientWait(Task):
    ...
    Args:
        ...
        - waiter_name (str, optional): The name of the waiter to instantiate. Can be a boto-supported
            waiter or one of merak's custom waiters. Currently, merak offers three additional
            ...
            find the definitions for all merak-defined waiters [here](https://github.com/merakHQ/merak/tree/master/src/merak/tasks/aws/waiters).  # noqa
    ...
    def run(
        ...
        if waiter_definition:
            ...
            # Use either boto-provided or merak-provided waiter
            if waiter_name in boto_client.waiter_names:
                waiter = boto_client.get_waiter(waiter_name)
            else:
                waiter = self._load_merak_waiter(boto_client, client, waiter_name)
