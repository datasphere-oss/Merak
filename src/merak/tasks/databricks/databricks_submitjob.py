...
from merak.exceptions import MerakException
from merak.tasks.databricks.databricks_hook import DatabricksHook
from merak.tasks.databricks.models import (
...
def _handle_databricks_task_execution(task, hook, log, submitted_run_id):
    """
    Handles the Databricks + merak lifecycle logic for a Databricks task

    Args:
        - task (merak.Task) : merak task being handled
        - hook (merak.tasks.databricks.databricks_hook.DatabricksHook): Databricks Hook
        - log (logger): merak logging instance
    ...
    while True:
        ...
        if run_state.is_terminal:
            if run_state.is_successful:
                ...
                raise merakException(error_message)
        else:
...
class DatabricksSubmitRun(Task):
    ...
    Args:
        - databricks_conn_secret (dict, optional): Dictionary representation of the Databricks Connection
            ...
            `merak__CONTEXT__SECRETS__DATABRICKS_CONNECTION_STRING=
        ...
        - run_name (str, optional): The run name used for this task.
            By default this will be set to the merak `task_id`. This `task_id` is a
    ...
    Examples:
        ...
        from merak.tasks.databricks import DatabricksSubmitRun
        ...
        json = {
            ...
            'notebook_task': {
                'notebook_path': '/Users/andrew.h@merak.io/records',
            },
        }

        with Flow("my flow") as flow:
            conn = merakSecret('DATABRICKS_CONNECTION_STRING')
        ...
        notebook_task = {
            'notebook_path': '/Users/merak@example.com/PrepareData',
        }
        ...
        This task requires a Databricks connection to be specified as a merak secret and can
    ...
    def run(
        ...
        if databricks_conn_secret is None:
            raise ValueError(
                ...
                "a dictionary or through merak Secrets"
            )
        ...
        if "run_name" not in self.json:
            self.json["run_name"] = run_name or "Run Submitted by merak"
...
class DatabricksRunNow(Task):
    ...
    Examples:
        ...
        from merak import Flow
        from merak.tasks.secrets import merakSecret
        from merak.tasks.databricks import DatabricksRunNow
...
class DatabricksSubmitMultitaskRun(Task):
    ...
    tasks are distinct from merak tasks. All tasks configured will run as a single merak task.
    ...
    Args:
        ...
        - run_name (str, optional): An optional name for the run.
            The default value is "Job run created by merak flow run {flow_run_name}".
    ...
    Examples:
        ...
        ```
        from merak import Flow
        from merak.tasks.databricks import DatabricksSubmitMultitaskRun
        from merak.tasks.databricks.models import (
            AccessControlRequestForUser,
            AutoScale,
            AwsAttributes,
    ...
    def run(
        ...
        Args:
            - databricks_conn_secret (dict, optional): Dictionary representation of the Databricks
                ...
                the key `host`. `merak__CONTEXT__SECRETS__DATABRICKS_CONNECTION_STRING=
                '{"host": "abcdef.xyz", "login": "ghijklmn", "password": "opqrst"}'`
                OR
                `merak__CONTEXT__SECRETS__DATABRICKS_CONNECTION_STRING=
                '{"host": "abcdef.xyz", "token": "ghijklmn"}'`
        ...
        run_name = (
            run_name
            or f"Job run created by merak flow run {merak.context.flow_run_name}"
        )
        # Ensures that multiple job runs are not created on retries
        idempotency_token = idempotency_token or merak.context.flow_run_id
