...
class ResourceContext:
    ...
    def __enter__(self) -> Optional[Task]:
        self.__prev_resource = merak.context.get("resource")
        merak.context.update(resource=self)
        return self.setup_task

    def __exit__(self, *args: Any) -> None:
        if self.__prev_resource is None:
            merak.context.pop("resource", None)
        else:
            merak.context.update(resource=self.__prev_resource)
...
class ResourceManager:
    ...
    ```python
    from merak import resource_manager
