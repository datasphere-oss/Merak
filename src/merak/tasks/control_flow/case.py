...
class case(object):
    ...
    def __enter__(self):
        self.__prev_case = merak.context.get("case")
        merak.context.update(case=self)

    def __exit__(self, *args):
        if self.__prev_case is None:
            merak.context.pop("case", None)
        else:
            merak.context.update(case=self.__prev_case)
