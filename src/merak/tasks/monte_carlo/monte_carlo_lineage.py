...
from merak.tasks.monte_carlo.client import MonteCarloClient
...
class MonteCarloCreateOrUpdateLineage(Task):
    ...
    Args:
        ...
        - api_key_id (string, optional): to use this task,
            ...
            in plain text, you can leverage the merakSecret task in your flow.
        - api_token (string, optional): the API token.
             To avoid passing the credentials in plain text,
             you can leverage merakSecret task in your flow.
        - merak_context_tags (bool, optional): whether to automatically add
            tags with merak context.
        ...
        source = dict(
            ...
            tags=[{"propertyName": "merak_test_key", "propertyValue": "merak_test_value"}]
        )
        ```
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.merak_context_tags = merak_context_tags
    ...
    def run(
        ...
        if the `merak_context_tags` flag is set to True (default),
        the task creates tags with information from merak context
        ...
        # source node
        if source.get("tags") is None and merak_context_tags is False:
        ...
        elif source.get("tags") is None and merak_context_tags is True:
            source_node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
                source["node_name"],
                source["object_id"],
                source["object_type"],
                source["resource_name"],
                tags=get_merak_context_tags(),
            )
        elif source.get("tags") is not None and merak_context_tags is False:
            source_node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
                source["node_name"],
                source["object_id"],
                source["object_type"],
            ...
            lineage_tags = [*source["tags"], *get_merak_context_tags()]
            source_node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
                source["node_name"],
                source["object_id"],
                source["object_type"],
        ...
        # destination node
        if destination.get("tags") is None and merak_context_tags is False:
        ...
        elif destination.get("tags") is None and merak_context_tags is True:
        ...
        elif destination.get("tags") is not None and merak_context_tags is False:
            destination_node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
                destination["node_name"],
                destination["object_id"],
                destination["object_type"],
            ...
            lineage_tags = [*destination["tags"], *get_merak_context_tags()]
            destination_node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
                destination["node_name"],
                destination["object_id"],
                destination["object_type"],
...
class MonteCarloCreateOrUpdateNodeWithTags(Task):
    ...
    If the `merak_context_tags` flag is set to True (default),
    the task additionally creates tags with merak
    ...
    Args:
        ...
        - api_token (string, optional): the API token.
             To avoid passing the credentials in plain text,
             you can leverage the merakSecret task in your flow.
        - merak_context_tags (bool, optional): whether to automatically add
            tags with merak context.
    ...
    def run(
        ...
        If the `merak_context_tags` flag is set to True (default), the task additionally creates
        a tag with the merak context dictionary for debugging data downtime issues.
        ...
        Args:
            ...
            - api_token (string, optional): the API token.
                 To avoid passing the credentials in plain text,
                 you can leverage the merakSecret task in your flow.
            - merak_context_tags (bool, optional): whether to automatically add a tag
                with merak context.
        ...
        mc = MonteCarloClient(api_key_id, api_token)
        if lineage_tags is None and merak_context_tags is False:
            ...
            if merak_context_tags is False:
                ...
                final_tags = [*lineage_tags, *get_merak_context_tags()]
                node_mcon = mc.create_or_update_lineage_node_with_multiple_tags(
...
class MonteCarloGetResources(Task):
    ...
    Args:
        - api_key_id (string, optional): to use this task, you need to pass
            ...
            in plain text, you can leverage the merakSecret task in your flow.
        - api_token (string, optional): the API token.
             To avoid passing the credentials in plain text, you can leverage the merakSecret task
             in your flow.
    ...
    def run(
        ...
        Args:
            - api_key_id (string, optional): to use this task,
                ...
                in plain text, you can leverage the merakSecret task in your flow.
            - api_token (string, optional): the API token.
                 To avoid passing the credentials in plain text, you can leverage the merakSecret
                 task in your flow.
...
def get_merak_context_tags() -> List[Dict[str, Any]]:
    """
    Helper function to generate a list of merak context tags
    when `merak_context_tags` is set to True.

    Returns:
        - list: a list of dictionaries with the task's context.
    """
    return [
        dict(propertyName="flow_id", propertyValue=str(merak.context.get("flow_id"))),
        dict(
            propertyName="flow_name",
            propertyValue=str(merak.context.get("flow_name")),
        ),
        dict(
            propertyName="flow_run_id",
            propertyValue=str(merak.context.get("flow_run_id")),
        ),
        dict(
            propertyName="flow_run_name",
            propertyValue=str(merak.context.get("flow_run_name")),
        ),
        dict(
            propertyName="flow_run_version",
            propertyValue=str(merak.context.get("flow_run_version")),
        ),
        dict(
            propertyName="project_id",
            propertyValue=str(merak.context.get("project_id")),
        ),
        dict(
            propertyName="project_name",
            propertyValue=str(merak.context.get("project_name")),
        ),
        dict(
            propertyName="scheduled_start_time",
            propertyValue=str(merak.context.get("scheduled_start_time")),
        ),
        dict(
            propertyName="execution_start_time",
            propertyValue=str(merak.context.get("date")),
        ),
        dict(propertyName="task_id", propertyValue=str(merak.context.get("task_id"))),
        dict(
            propertyName="task_run_id",
            propertyValue=str(merak.context.get("task_run_id")),
        ),
        dict(
            propertyName="task_run_name",
            propertyValue=str(merak.context.get("task_run_name")),
        ),
        dict(
            propertyName="task_slug",
            propertyValue=str(merak.context.get("task_slug")),
        ),
        dict(
            propertyName="task_name",
            propertyValue=str(merak.context.get("task_name")),
        ),
        dict(
            propertyName="task_full_name",
            propertyValue=str(merak.context.get("task_full_name")),
        ),
        dict(
            propertyName="task_run_count",
            propertyValue=str(merak.context.get("task_run_count")),
        ),
        dict(
            propertyName="map_index",
            propertyValue=str(merak.context.get("map_index")),
        ),
        dict(
            propertyName="parameters",
            propertyValue=str(merak.context.get("parameters")),
        ),
        dict(
            propertyName="running_with_backend",
            propertyValue=str(merak.context.get("running_with_backend")),
        ),
    ]
