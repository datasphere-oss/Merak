...
try:
    ...
    raise ImportError(
        "Using `merak.tasks.secrets.vault_secret` requires hvac to be installed."
    ) from err

import merak
from merak.tasks.secrets import SecretBase, merakSecret
...
class VaultSecret(SecretBase):
    ...
    Args:
        ...
        - vault_credentials_secret (str, optional): name of the merakSecret containing Vault
          credentials.
            Defaults to a merakSecret named `VAULT_CREDENTIALS`.
    ...
    def _get_vault_secret(self, name: str):
        ...
        # get vault auth credentials from the merakSecret
        vault_creds = merakSecret(self.vault_credentials_secret).run()
