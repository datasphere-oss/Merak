from merak.client.secrets import Secret as _Secret
from merak.core.task import Task
from merak.engine.results import SecretResult
...
class merakSecret(SecretBase):
    """
    merak Secrets Task.  This task retrieves the underlying secret through
    the merak Secrets API (which has the ability to toggle between local vs. Cloud secrets).
    ...
    def run(self, name: str = None):
        ...
        merak Cloud, depending on whether `config.cloud.use_local_secrets` is `True` or
        `False`.
        ...
        Returns:
            - Any: the underlying value of the merak Secret
        """
        if name is None:
