...
class CensusSyncTask(Task):
    ...
    sync using merak task to send a post to the API within a merak flow. Copy and paste from the api
    ...
    Example:
        Trigger a Census sync with an `api_trigger` stored in a merak secret:
        ```python
        from merak import Flow
        from merak.tasks.census import CensusSyncTask
        from merak.tasks.secrets import merakSecret

        sync_census = CensusSyncTask()

        with Flow("Run Census Sync") as flow:
            api_trigger = merakSecret('CENSUS_API_TRIGGER')
