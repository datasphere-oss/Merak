...
class DaskExecutor(Executor):
    ...
    Args:
        ...
        - disable_cancellation_event (bool, optional): By default, merak uses a
            ...
            skip this check.
        - watch_worker_status (bool, optional): By default, merak subscribes to Dask
    ...
    def _pre_start_yield(self) -> None:
        ...
        if (
            ...
            self._should_run_event = Event(
                f"merak-{uuid.uuid4().hex}", client=self.client
            )
...
class LocalDaskExecutor(Executor):
    ...
    def start(self) -> Iterator:
        """Context manager for initializing execution."""
        # import dask here to reduce merak import times
        import dask.config
        from dask.callbacks import Callback
        from dask.system import CPU_COUNT

        class merakCallback(Callback):
        ...
        with merakCallback(), dask.config.set(**self.dask_config):
    ...
    def wait(self, futures: Any) -> Any:
        ...
        if self.scheduler == "processes":
            ...
            def patch() -> Iterator[None]:
                # Patch around https://github.com/merakHQ/merak/issues/4086
        ...
        # Patch around https://github.com/merakHQ/merak/issues/4537
