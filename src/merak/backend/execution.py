...
def execute_flow_run_in_subprocess(
    ...
    Args:
        ...
        - run_api_key: The authentication key to provide to the flow run for
            communicating with the merak Cloud API. If not set, it will be inferred
    ...
    while not flow_run.state.is_finished():
        ...
        try:
            ...
            # the flow run loads a correct merak config. This CLI command calls out to
            # `execute_flow_run`
            logger.info("Creating subprocess to execute flow run...")
            result = subprocess.run(
                [sys.executable, "-m", "merak", "execute", "flow-run"], env=env
            )
...
def execute_flow_run(
    flow_run_id: str,
    flow: "Flow" = None,
    runner_cls: Type["merak.engine.flow_runner.FlowRunner"] = None,
    **kwargs: Any,
) -> "FlowRunView":
    """
    ...
    runner_cls = runner_cls or merak.engine.cloud.flow_runner.CloudFlowRunner
    ...
    if flow_metadata.storage:
        ...
        for secret in flow_metadata.storage.secrets:
            with _fail_flow_run_on_exception(
                ...
                secrets[secret] = merak.tasks.secrets.merakSecret(name=secret).run()
    ...
    with merak.context(flow_run_id=flow_run_id):
        with _fail_flow_run_on_exception(
            flow_run_id=flow_run_id,
...
def generate_flow_run_environ(
    ...
    client = merak.Client()  # Instantiate a client to get the current API key
    env["merak__CLOUD__API_KEY"] = run_api_key or client.api_key or ""
    # Backwards compat for auth tokens (only useful for containers)
    env["merak__CLOUD__AUTH_TOKEN"] = run_api_key or client.api_key or ""

    # Add context information for the run
    env.update(
        {
            "merak__CONTEXT__FLOW_RUN_ID": flow_run_id,
            "merak__CONTEXT__FLOW_ID": flow_id,
        }
    )
...
def _fail_flow_run(flow_run_id: str, message: str) -> None:
    ...
    client.set_flow_run_state(
        flow_run_id=flow_run_id, state=merak.engine.state.Failed(message)
    )
    client.write_run_logs(
        [
            dict(
                flow_run_id=flow_run_id,  # type: ignore
                name="merak.backend.execution",
                message=message,
                level="ERROR",
            )
        ]
