...
import merak
from merak.exceptions import (
    AuthorizationError,
    ClientError,
    ObjectNotFoundError,
    VersionLockMismatchSignal,
)
from merak.run_configs import RunConfig
from merak.utilities.graphql import (
    EnumValue,
    GraphQLResult,
    compress,
...
from merak.utilities.logging import create_diagnostic_logger

if TYPE_CHECKING:
    import requests

    from merak.core import Flow
...
class TaskRunInfoResult(NamedTuple):
    # TODO: Deprecate this result in favor of `merak.backend.TaskRun`
    id: str
    task_id: str
    task_slug: str
    version: int
...
class FlowRunInfoResult(NamedTuple):
    # TODO: Deprecate this result in favor of `merak.backend.FlowRun`
    id: str
    name: str
    flow_id: str
    project: ProjectInfo
...
class Client:
    """
    Client for communication with merak Cloud

    If the arguments aren't specified the client initialization first checks the merak
    ...
    Args:
        - api_server (str, optional): the URL to send all GraphQL requests to; if not
            ...
            variable
        - api_key (str, optional): a merak Cloud API key. If not provided, loaded
            from `config.cloud.api_key` or from the on disk cache from the
            `merak auth` CLI
        - tenant_id (str, optional): the merak tenant to use. If not provided, loaded
    ...
    def __init__(
        ...
        self._auth_file = Path(merak.context.config.home_dir).absolute() / "auth.toml"
        ...
        self.api_server = api_server or merak.context.config.cloud.api

        # Load the API key
        cached_auth = self.load_auth_from_disk()
        self.api_key: Optional[str] = (
            api_key
            or merak.context.config.cloud.get("api_key")
        ...
        self.tenant_id: Optional[str] = (
            ...
            or merak.context.config.cloud.get("tenant_id")
    ...
    def _get_auth_tenant(self) -> str:
        ...
        """
        if not merak.config.backend == "cloud":

            raise ValueError(
                "Authentication is only supported for merak Cloud. "
                "Your backend is set to {merak.config.backend!r}"
            )
    ...
    def _get_default_server_tenant(self) -> str:
        if merak.config.backend == "server":
            ...
            if not tenants:  # The user has not created a tenant yet
                raise ClientError(
                    "Your merak Server instance has no tenants. "
                    "Create a tenant with `merak server create-tenant`"
                )

            return tenants[0].id

        elif merak.config.backend == "cloud":
            raise ValueError(
                "Default tenants are determined by authentication in merak Cloud. "
                "See `_get_auth_tenant` instead."
            )
        else:
            raise ValueError("Unknown backend {merak.config.backend!r}")
    ...
    def save_auth_to_disk(self) -> None:
        ...
        self._auth_file.write_text(
            ...
            "# Update the merak config or use the CLI to login instead\n\n"
            + toml.dumps(contents)
        )
    ...
    def tenant_id(self) -> str:
        ...
        if not self._tenant_id:
            ...
            elif merak.config.backend == "server":
                self._tenant_id = self._get_default_server_tenant()
            else:
                raise ValueError(f"Unknown backend setting {merak.config.backend!r}")

        if not self._tenant_id:
            raise ClientError(
                "A tenant could not be determined. Please use `merak auth status` "
    ...
    def create_tenant(self, name: str, slug: str = None) -> str:
        ...
        Note this route only works when run against merak Server.
        ...
        Raises:
            - ValueError: if run against merak Cloud
        """
        if merak.config.backend != "server":
            raise ValueError(
                "To create a tenant with merak Cloud, please signup at "
                "https://cloud.merak.io/"
            )
    ...
    def get(
        ...
        """
        Convenience function for calling the merak API with auth and GET request

        Args:
            - path (str): the path of the API url. For example, to GET
                http://merak-server/v1/auth/login, path would be 'auth/login'.
    ...
    def post(
        ...
        """
        Convenience function for calling the merak API with auth and POST request
    ...
    def graphql(
        ...
        """
        Convenience function for running queries against the merak GraphQL API

        Args:
            - query (Any): A representation of a graphql query to be executed. It will be
                parsed by merak.utilities.graphql.parse_graphql().
    ...
    def _send_request(
        ...
        if merak.context.config.cloud.get("diagnostics") is True:
        ...
        if method == "GET":
            response = session.get(
                ...
                headers=headers,
                params=params,
                timeout=merak.context.config.cloud.request_timeout,
            )
        elif method == "POST":
    ...
    def _request(
        ...
        if self.api_key and self._tenant_id:
            ...
            headers["X-merak-TENANT-ID"] = self._tenant_id

        headers["X-merak-CORE-VERSION"] = str(merak.__version__)
        ...
        try:
            ...
            if merak.config.backend == "cloud" and "Authorization" not in headers:
                raise AuthorizationError(
                    "Malformed response received from Cloud - please ensure that you "
                    "are authenticated. See `merak auth login --help`."
                ) from exc
            else:
    ...
    def register(
        ...
        """
        Register a new flow with merak Cloud.
        ...
        if not project:
            raise ValueError(
                "Project {} not found. Run `merak create project '{}'` to create it.".format(
        ...
        try:
            merak.serialization.flow.FlowSchema().load(serialized_flow)
    ...
    def get_cloud_url(self, subdirectory: str, id: str) -> str:
        """
        Convenience method for creating merak Cloud URLs for a given subdirectory.
        ...
        ```python
        from merak import Client

        client = Client()
        client.get_cloud_url("flow-run", "424242-ca-94611-111-55")
        # returns "https://cloud.merak.io/my-tenant-slug/flow-run/424242-ca-94611-111-55"
        ```
        """

        # Search for matching cloud API because we can't guarantee that the backend config is set
        using_cloud_api = ".merak.io" in merak.config.cloud.api
        tenant_slug = self.get_default_tenant_slug()

        # For various API versions parse out `api-` for direct UI link
        base_url = (
            (
                re.sub("api-", "", merak.config.cloud.api)
                if re.search("api-", merak.config.cloud.api)
                else re.sub("api", "cloud", merak.config.cloud.api)
            )
            if using_cloud_api
            else merak.config.server.ui.endpoint
        )
    ...
    def get_flow_run_info(self, flow_run_id: str) -> FlowRunInfoResult:
        ...
        task_runs = [
            TaskRunInfoResult(
                ...
                else merak.engine.state.Pending(),
            )
            for tr in result.task_runs
        ]

        state = (
            merak.engine.state.State.deserialize(result.serialized_state)
    ...
    def update_flow_run_heartbeat(self, flow_run_id: str) -> None:
        ...
        self.graphql(
            mutation,
            raise_on_error=True,
            headers={"X-merak-HEARTBEAT-ID": flow_run_id},
        )
    ...
    def get_flow_run_state(self, flow_run_id: str) -> "merak.engine.state.State":
        """
        ...
        return merak.engine.state.State.deserialize(flow_run.serialized_state)
    ...
    def set_flow_run_state(
        ...
        if state_payload.status == "QUEUED":
            ...
            return merak.engine.state.Queued(
                message=state_payload.get("message"),
                start_time=pendulum.now("UTC").add(
                    seconds=merak.context.config.cloud.queue_interval
                ),
            )
        return state
    ...
    def get_latest_cached_states(
        ...
        deserializer = merak.engine.state.State.deserialize
        valid_states = [
    ...
    def get_task_run_info(
        ...
        state = (
            merak.engine.state.State.deserialize(task_run_info.serialized_state)
    ...
    def get_task_run_state(self, task_run_id: str) -> "merak.engine.state.State":
        """
        ...
        return merak.engine.state.State.deserialize(task_run.serialized_state)
